...
class Route53Stack(Stack):
    def __init__(self, scope: Construct, id: str, searxng_stacks: dict, **kwargs) -> None:
        ...
        try:
            ...
            hosted_zone = route53.PublicHostedZone(
                self, "SearxngHostedZone",
                zone_name=domain_name,
                comment="Hosted zone for SearxNG latency-based routing"
            )

        # 为每个区域的 Lambda URL 创建记录
        for region, stack in searxng_stacks.items():
            # 创建延迟路由记录
            route53.CnameRecord(
